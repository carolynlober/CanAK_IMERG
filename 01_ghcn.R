...
# set directory 
ghcn_loc <- here("precip_data", "GHCN", "daily-summaries-latest_12.30")
...
# ghcn metadata
ghcn_stations_meta <- read.csv(here("precip data", "GHCN", "ghcnd-stations.csv"),header=F)
ghcn_stations_meta$V1 <- as.character(ghcn_stations_meta$V1)

# list files
ghcn_fnames <- list.files(ghcn_loc, full.names = TRUE)
...
to_remove <- list.files(ghcn_loc)[which(!(substr(list.files(),1,11) %in% to_keep))]
...
# define bounds of PAD study area
xmin <- -120.3
xmax <- -96.74
ymin <- 54.58
ymax <- 62.84
...
# get file names
ghcn_fnames <- paste0(ghcn_loc, "/", pad_coords$file, ".csv")
...
for(x in pad_list[1:length(pad_list)]){
  if ("PRCP" %in% colnames(x)) {
    ...
    cat(paste0(x$STATION[1], " is done "))
  }
  else {
    cat(paste0(x$STATION[1], " has no precipitation data"))
  } 
}
...
pad_ghcn_data <- read.csv("precip_data", "GHCN", "pad_stations_data.csv")
