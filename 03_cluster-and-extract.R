...
my_crs <- crs(readOGR("/Volumes/GoogleDrive/My\ Drive/senior/thesis/gis/big_study.shp"))
...
ghcn_hclust40 <- SpatialPointsDataFrame(ghcn_meta[,c(4,3)],
                                        ghcn_hclust40,
                                        proj4string = my_crs)
writeOGR(ghcn_hclust40, "/Volumes/GoogleDrive/My\ Drive/senior/thesis/gis/",
...
ghcn_data_clust <- aggregate(ghcn_data,by=list(ghcn_meta$hclust_40),FUN=function(x){mean(x,na.rm=T)})
...
ghcn_hclust40_reproj <- spTransform(ghcn_hclust40,crs(above_300m[[1]]))
...
buffered_hulls_spdf <- SpatialPolygonsDataFrame(buffered_hulls[[1]],data=data,match.ID=F)
for(x in 2:40) {
  data <- ghcn_meta_clust[order_ndx[x],]
  spdf <- SpatialPolygonsDataFrame(buffered_hulls[[x]],data=data,match.ID=F)
...
## ------------------------------------------------------------ ## 
## EXTRACT DAILY IMERG DATA AT BUFFERED HULLS (later)
...
imerg_data_clust <- sapply(imerg_fnames,function(x){
  # open file
  scene <- raster(x)
  # extract data
  return(raster::extract(x=scene,buffered_hulls,fun=mean,na.rm=T))
  # at end of year, print that the year is done
  if (substr(x,26,29) == "1231") {
    cat(paste(substr(x,22,25)," is done! ")) 
  }
})
write.table(imerg_data_clust,"/Volumes/GoogleDrive/My\ Drive/senior/thesis/imerg_data_clust.csv")

imerg_data_clust <- as.data.frame(imerg_data_clust)
colnames(imerg_data_clust) <- (paste(substr(imerg_fnames,22,25),substr(imerg_fnames,26,27),substr(imerg_fnames,28,29),sep="-"))
