...
p_tmp <- ggplot(data_tmp, aes(x=dates,y=data,col=dates)) +
  geom_point() + scale_colour_gradientn(colours = rainbow(7), limits=c(200006,202012),breaks=c(200006,202012))
...
relative_bias <- function(s,r){ # s = estimate, r = parameter
  ...
  if (length(s) != length(r)){
    ...
    mean(sapply(1:length(s),function(x){(s[x]-r[x])*2/(e+s[x]+r[x])}),na.rm=T)
  }
}

# calculate bias for all of the clusters
bias_total <- imerg_months - ghcn_months 
bias_all_clust <- apply(bias_total,1,function(x){mean(x,na.rm=T)})
...
rbias_all_clust <- sapply(c(1:40),function(x){
  ...
  mean(relative_bias(as.numeric(imerg_months[ndx2,no_na]),as.numeric(ghcn_months[ndx1,no_na])),na.rm=T)
})
...
bias_daily_all_clust <- apply(bias_daily_total,1,function(x){mean(x,na.rm=T)})
...
rbias_daily_all_clust <- sapply(c(1:40),function(x){
  ...
  mean(relative_bias(as.numeric(imerg_data_clust[ndx2,no_na]),as.numeric(ghcn_data_clust[ndx1,no_na])),na.rm=T)
})
...
monthly_bias <- sapply(c(1:12),function(x){
  apply(bias_total,1,function(y){
    mean(y[which(col_months == x)],na.rm=T)
  })
})
...
calc_pod <- function(ghcn,imerg){ 
  TP <- sum(imerg == 1 & ghcn == 1,na.rm=T)
  FN <- sum(imerg == 0 & ghcn == 1,na.rm=T)
  return(TP/(TP+FN))
}
...
m_e <- ggplot(data=small_df_m, aes(x=merit,y=bias,col=month)) +
  geom_abline(slope=0,intercept=0) +
  geom_smooth(method="loess",se=T, level=0.5) +
  xlab("Elevation") +
  ylab("Bias") +
